...
export function RecordForm({ projectId, kind }) {
  ...
  // รีเซ็ตเมื่อเปลี่ยนโครงการ/แท็บ
  useEffect(() => {
    const now = nowLocalParts();
    setDatePart(now.datePart);
    setTimePart(now.timePart);
    setAmount(""); setSource(""); setFromWho(""); setReceiver("");
    setItemName(""); setCategory("วัสดุ"); setPayer(""); setVendor(""); setInvoiceNo("");
  ...
  async function onSubmit(e) {
    ...
    setDatePart(now.datePart);
    setTimePart(now.timePart);
    setAmount(""); setSource(""); setFromWho(""); setReceiver(""); setItemName(""); setPayer("");
    setVendor(""); setInvoiceNo(""); setNote(""); setFile(null);
    if (fileRef.current) fileRef.current.value = "";
  }
  ...
  return (
    <form onSubmit={onSubmit} className="card grid gap-3 mb-4">
      ...
      <button className="btn btn-primary">บันทึก{kind === "incomes" ? "รายรับ" : "รายจ่าย"}</button>
    </form>
  );
}
